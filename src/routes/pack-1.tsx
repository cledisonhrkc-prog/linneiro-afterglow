...
export const Route = createFileRoute("/pack-1")({ component: Pack1 });

const PACK = SONGS.filter((s) => s.no <= 10);
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 pt-28 pb-10 md:px-8">
      <p className="text-xs tracking-[0.28em] text-muted uppercase">
        Pack 1 · um de cada vez
      </p>
      <h1 className="mt-2 font-display text-4xl">
        {n}. {song.title}
      </h1>
      <p className="mt-2 text-sm text-muted">{i + 1} de 10</p>
      ...
      <div className="mt-6 flex items-center justify-between gap-3">
        ...
        <span className="text-sm tabular-nums text-muted">{n} / 10</span>
        <button
          type="button"
